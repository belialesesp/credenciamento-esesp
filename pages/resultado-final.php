...
function exportToExcel() {
    ...
    csvContent += "Tipo,Curso,Nome,Chamado em,Inscrito em\n";
    ...
    allTeachersData.interpreters.forEach(interpreter => {
        csvContent += `"Intérprete","N/A","${interpreter.name}","---","${new Date(interpreter.created_at).toLocaleDateString('pt-BR')}"\n`;
    });
    
    // Add technicians
    allTeachersData.technicians.forEach(technician => {
        csvContent += `"Técnico","N/A","${technician.name}","---","${new Date(technician.created_at).toLocaleDateString('pt-BR')}"\n`;
    });
    ...
    link.setAttribute("download", "profissionais_aptos_" + new Date().toISOString().split('T')[0] + ".csv");
