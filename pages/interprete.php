<?php
// pages/interprete.php - Fixed to match tecnico.php structure
...
if (!$stmt->fetch()) {
  if ($is_ajax_request) {
    echo '<div class="alert alert-danger">Intérprete não encontrado.</div>';
    exit();
  } else {
...
try {
  $interpreter = $interpreterService->getInterpreter($requested_id);
  ...
  // Get evaluation data from interpreter table  
  $eval_sql = "SELECT gese_evaluation, pedagogico_evaluation FROM interpreter WHERE id = (
        SELECT type_id FROM user WHERE id = ? AND user_type = 'interpreter'
    )";
  ...
  if ($is_ajax_request) {
    echo '<div class="alert alert-danger">Erro ao carregar dados do intérprete: ' . htmlspecialchars($e->getMessage()) . '</div>';
    exit();
  } else {
    ob_start();
    include '../components/header.php';
    echo '<div class="container">
                <h1 class="main-title">Erro</h1>
                <p>Erro ao carregar dados do intérprete: ' . htmlspecialchars($e->getMessage()) . '</p>
                <a href="interpretes.php" class="btn btn-primary">Voltar para lista de intérpretes</a>
              </div>';
    include '../components/footer.php';
    ob_end_flush();
...
<div class="container container-user">
  ...
  <div class="info-section">
    ...
    <?php if (!empty($file_path)): ?>
      <a href="../backend/documentos/interpretes/<?= $file_path ?>" target="_blank">Download</a>
    <?php else: ?>
