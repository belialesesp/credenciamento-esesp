...
require_once "../backend/services/spc_teacher.service.php";
...
if(isset($_GET["id"])) {
  ...
  $teacherService = new SpcTeacherService($conn);
  
  $teacher = $teacherService->getSpcDemandTeacher($teacher_id);
  ...
  foreach($courses as $course) {
    $courses_str .= 'Curso: ' . $course->name . ' - Órgão: ' . $course->institution . '<br>';
  }
  ...
  $html = file_get_contents("templateDocente.html");
  $html = str_replace(
    [
      "{{date}}",
      "{{name}}",
  ...
  $pdf_name = __DIR__ . '/fichasDemandas/' . $name . '.pdf';
