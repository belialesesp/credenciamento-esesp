...
function post_postg_teacher($address_id, $conn) {
  ...
  if($query_execute) {
    ...
    $response = [
      'success' => true,
      'redirect_url' =>  "https://credenciamento.esesp.es.gov.br/credenciamento/pages/sucesso.php?category=docentepos&id=$id"
    ];
...
function post_certificate($conn, $teacher_id) {
  ...
  if (isset($_FILES['documents']) && $_FILES['documents']['error'] == 0) {
    ...
    $uploadDir = realpath(__DIR__ . '/../documentos/posgraduacao/') . '/';
