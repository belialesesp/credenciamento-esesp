...
function create_user_account($conn, $interpreter_id, $name, $email, $cpf, $user_type) {
    try {
        // Clean CPF
...
function post_interpreter($conn, $address_id) {
  $name = $_POST["name"];
  $cpf = $_POST["cpf"];
  ...
  if($query_execute) {
    ...
    $response = [
      'success' => true,
      'redirect_url' =>  "https://credenciamento.esesp.es.gov.br/credenciamento/pages/sucesso.php?category=interprete&id=$id"
    ];
...
function post_documentation($conn, $interpreter_id) {
  ...
  if (isset($_FILES['documents']) && $_FILES['documents']['error'] == 0) {
    ...
    $uploadDir = realpath(__DIR__ . '/../documentos/interpretes/') . '/';
